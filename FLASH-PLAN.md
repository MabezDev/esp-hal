...
| `Flash::new(FLASH, Config)` | Construction reads the cached three-byte JEDEC ID and decodes it; **it issues no flash command** and does not refresh the cache (design [B2](FLASH-DESIGN.md#b2-the-cached-jedec-id-and-its-provenance)). A sentinel, the ROM's W25Q16 default, or an unsupported density returns `ConfigError::UnknownFlashChip`; it does not panic and never yields a capacity-0 driver (design [A13](FLASH-DESIGN.md#a13-no-chip-configuration-at-launch)) |
...
**Scope**
- `esp-rom-sys`: add a uniform accessor for the ROM-cached flash metadata. ESP32/ESP32-S2 expose the structure directly as `g_rom_flashchip`; the other nine targets expose `rom_spiflash_legacy_data`, which is a **pointer** to it, so the accessor needs one extra indirection there (design [B2](FLASH-DESIGN.md#b2-the-cached-jedec-id-and-its-provenance)). No `esp_rom_spi_flash_update_id` binding is needed: the driver reads the cache and never refreshes it. Move `esp-hal/src/psram/esp32.rs`, which already declares the struct and reads `device_id`, onto the shared accessor.
...
- Capacity detection reads the cached `device_id` and decodes physical capacity with the esptool-derived table. It issues no flash command, so `new` needs no operation guard and no `#[ram]`. Identification is the boot chain's job; recomputing it would mean owning a detection path no single ROM function provides (design [Why the driver does not read the ID itself](FLASH-DESIGN.md#why-the-driver-does-not-read-the-id-itself)).
...
- `#[ram]` goes on every method that can cause a SPI1 transaction or touch the cache, plus everything on that path. Not on `new`, `apply_config`, `capacity` or `chip_info`; reading a field that describes the peripheral is not a flash operation (design [A18](FLASH-DESIGN.md#a18-ram-residency-scoped-by-flash-capability)). PSRAM-backed stacks remain documented as unsupported.
...
- Extend `hil-test/src/bin/flash.rs` with encryption-off reads and encrypted writes behind an explicit test-only bypass of the production guard, the same `__test_esp_storage`-style cfg `esp-storage/src/encrypted.rs:73` uses today. Test chunk boundaries, erased-write behavior, alignment rejection, no implicit erase, and production `NotSupported`. On ESP32 specifically, exercise a 16-byte-aligned but not 32-byte-aligned write at both the start and the end of a range. No CI device has encryption eFuses burned, so a true encrypted round-trip remains out of fleet scope.

### PR D1 — Bootloader flash seam + host-test decoupling (size: M) — independent of A/B/C; lands any time before D2

The partition access code is hardwired to the concrete `esp_storage::FlashStorage<'d>` (struct field `partitions.rs:618`, `as_flash_region`, unconditional `esp_storage` references) — there is no seam to swap behind, so cutting one is real refactor work on a published crate, not an afterthought of the swap. Do it while still esp-storage-backed, so the host-test churn is reviewed in isolation and D2 can focus on the public partition-wrapper correction.

- ~~Fix the gate itself~~: **done upstream.** `cargo xtask host-tests` used to run the bootloader with only `std`, compiling `nor_flash_tests` out; it now passes `["std", "embedded-storage"]`, so the current surface is already covered before D2 replaces it. Nothing to do here beyond confirming it stays enabled.
...
- Flip `stable = true` on FLASH in all 11 `soc.toml`s + `update-metadata`.
...
8. `cargo xtask host-tests` — requires the D1 mock decoupling; the xtask `embedded-storage` feature is already enabled upstream
...
14. HIL: capacity decoded from the cached ID equals the known board capacity on every family, including an octal boot mode; on ESP32 the cached ID is correct under the ESP-IDF bootloader; a sentinel, the ROM W25Q16 default, or an unsupported density returns `Err(ConfigError::UnknownFlashChip)` (design [B2](FLASH-DESIGN.md#b2-the-cached-jedec-id-and-its-provenance); first run in PR A)
...
- [ ] host-tests green on the bootloader's mock, with `embedded-storage` still enabled in the xtask run
...
| `esp-rom-sys/src/rom/spiflash.rs` | Add a cross-target accessor for ROM-cached flash metadata (pointer indirection on the 9 non-ESP32/S2 targets); reconcile the `esp_rom_spiflash_write_encrypted` 32-byte doc comment with the chosen public contract | A (+ C) |
...
| `esp-metadata/devices/*/soc.toml` (×11) | Add `[device.flash]` driver entry (README matrix row) | A (+ F: `stable = true` on FLASH) |
...
| `xtask` (host-tests invocation) | **No change needed** — `embedded-storage` is already enabled upstream | — |
