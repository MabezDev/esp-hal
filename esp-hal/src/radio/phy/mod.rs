...
pub(crate) const PHY_CALIBRATION_DATA_LENGTH: usize =
    core::mem::size_of::<crate::radio::sys::include::esp_phy_calibration_data_t>();
...
impl PhyState {
    ...
    fn calibrate(&mut self) {
        #[cfg(esp32s2)]
        unsafe {
            use crate::hal::efuse::Efuse;
            crate::radio::sys::include::phy_eco_version_sel(Efuse::major_chip_version());
        }
        ...
        let calibration_mode = if calibration_data_available {
            ...
            {
                crate::radio::sys::include::esp_phy_calibration_mode_t_PHY_RF_CAL_NONE
            } else if cfg!(phy_full_calibration) {
                crate::radio::sys::include::esp_phy_calibration_mode_t_PHY_RF_CAL_FULL
            } else {
                crate::radio::sys::include::esp_phy_calibration_mode_t_PHY_RF_CAL_PARTIAL
            }
        } else {
            crate::radio::sys::include::esp_phy_calibration_mode_t_PHY_RF_CAL_FULL
        };
        ...
        unsafe {
            self.calibration_result = crate::radio::sys::include::register_chipv7_phy(
                init_data,
    ...
    fn backup_digital_regs(&mut self) {
        unsafe {
            crate::radio::sys::include::phy_dig_reg_backup(
                true,
    ...
    pub fn increase_ref_count(&mut self) {
        if self.ref_count == 0 {
            ...
            if self.calibrated {
                unsafe {
                    crate::radio::sys::include::phy_wakeup_init();
                }
                #[cfg(phy_backed_up_digital_register_count_is_set)]
    ...
    pub fn decrease_ref_count(&mut self) {
        ...
        if self.ref_count == 0 {
            ...
            unsafe {
                // Disable PHY and RF.
                crate::radio::sys::include::phy_close_rf();

                // Power down PHY temperature sensor.
                #[cfg(not(esp32))]
                crate::radio::sys::include::phy_xpd_tsens();
            }
            #[cfg(esp32)]
            {
...
macro_rules! impl_phy_controller {
    ($feature_gate:ident, $peripheral:tt) => {
        #[cfg($feature_gate)]
        impl private::Sealed for crate::hal::peripherals::$peripheral<'_> {}

        #[cfg($feature_gate)]
        impl<'d> PhyController<'d> for crate::hal::peripherals::$peripheral<'d> {}
    };
}
...
#[cfg(esp32)]
impl MacTimeExt for crate::hal::peripherals::WIFI<'_> {}
...
pub fn backup_phy_calibration_data(
    ...
    PHY_STATE.with(|phy_state| {
        phy_state
            ...
            .map(|calibration_data| buffer.copy_from_slice(&calibration_data[..]))
    })
}
