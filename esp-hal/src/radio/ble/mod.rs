...
unstable_module! {
    /// Bluetooth Low Energy controller implementation.
    pub mod controller;
}

#[allow(dead_code)]
pub(crate) unsafe extern "C" fn malloc(size: u32) -> *mut super::sys::c_types::c_void {
    unsafe { super::compat::malloc::malloc(size as usize).cast() }
}

#[cfg(any(esp32, esp32c3, esp32s3))]
#[allow(dead_code)]
pub(crate) unsafe extern "C" fn malloc_internal(size: u32) -> *mut super::sys::c_types::c_void {
    unsafe { super::compat::malloc::malloc_internal(size as usize).cast() }
}

pub(crate) unsafe extern "C" fn free(ptr: *mut super::sys::c_types::c_void) {
    unsafe { super::compat::malloc::free(ptr.cast()) }
}
...
pub fn mac() -> [u8; 6] {
    let mut mac = [0u8; 6];
    unsafe {
        super::common_adapter::read_mac(mac.as_mut_ptr(), 2);
    }
    mac
}
