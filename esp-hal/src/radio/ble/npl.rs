...
static G_OSI_FUNCS: ExtFuncsT = ExtFuncsT {
    ...
    malloc: Some(crate::radio::ble::malloc),
    free: Some(crate::radio::ble::free),
    #[cfg(esp32c2)]
...
unsafe extern "C" fn os_random() -> u32 {
    trace!("os_random");
    unsafe { crate::radio::common_adapter::random() as u32 }
}
...
unsafe extern "C" fn task_delete(task: *mut c_void) {
    ...
    unsafe {
        crate::radio::preempt::schedule_task_deletion(NonNull::new(task.cast::<()>()));
    }
}
...
unsafe extern "C" fn ble_npl_hw_exit_critical(mask: u32) {
    ...
    unsafe {
        ...
        crate::radio::ESP_RADIO_LOCK.release(token);
    }
}

unsafe extern "C" fn ble_npl_hw_enter_critical() -> u32 {
    trace!("ble_npl_hw_enter_critical");
    unsafe { crate::radio::ESP_RADIO_LOCK.acquire().inner() }
}
...
unsafe extern "C" fn ble_npl_time_delay(time: ble_npl_time_t) {
    ...
    crate::radio::preempt::usleep(time);
}
...
unsafe extern "C" fn ble_npl_event_init(
    ...
    if unsafe { (*event).dummy } == 0 {
        unsafe {
            let evt = crate::radio::compat::malloc::calloc(1, core::mem::size_of::<Event>()) as *mut Event;
...
unsafe extern "C" fn ble_npl_callout_init(
    ...
    if unsafe { (*callout).dummy } == 0 {
        ...
        unsafe {
            let new_callout =
                crate::radio::compat::malloc::calloc(1, core::mem::size_of::<Callout>()) as *mut Callout;
...
pub(crate) fn ble_init(config: &Config) -> PhyInitGuard<'static> {
    ...
    unsafe {
        ...
        let mut mac = [0u8; 6];
        crate::radio::common_adapter::read_mac(mac.as_mut_ptr(), 2);
        mac.reverse();
