...
pub unsafe extern "C" fn read_mac(mac: *mut u8, type_: u32) -> c_int {
    ...
    let base_mac = crate::efuse::Efuse::mac_address();
...
pub unsafe extern "C" fn ets_timer_disarm(timer: *mut c_void) {
    super::compat::timer_compat::compat_timer_disarm(timer.cast());
}

#[cfg(feature = "wifi")]
pub unsafe extern "C" fn ets_timer_done(timer: *mut c_void) {
    super::compat::timer_compat::compat_timer_done(timer.cast());
}
...
pub unsafe extern "C" fn ets_timer_setfn(
    ...
    unsafe {
        super::compat::timer_compat::compat_timer_setfn(
            ptimer.cast(),
...
pub unsafe extern "C" fn ets_timer_arm(timer: *mut c_void, ms: u32, repeat: bool) {
    super::compat::timer_compat::compat_timer_arm(timer.cast(), ms, repeat);
}

#[cfg(feature = "wifi")]
pub unsafe extern "C" fn ets_timer_arm_us(timer: *mut c_void, us: u32, repeat: bool) {
    super::compat::timer_compat::compat_timer_arm_us(timer.cast(), us, repeat);
}
...
pub unsafe extern "C" fn __esp_radio_esp_timer_get_time() -> i64 {
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(feature = "wifi", feature = "ble"))] {
            super::preempt::now() as i64
        } else {
            // In this case we don't have a scheduler, we can return esp-hal's timestamp.
            crate::time::Instant::now().duration_since_epoch().as_micros() as i64
        }
    }
}
...
#[cfg(not(esp32h2))]
type ModemClockControllerPeripheral = crate::peripherals::WIFI<'static>;
#[cfg(esp32h2)]
type ModemClockControllerPeripheral = crate::peripherals::BT<'static>;
    ...
    #[cfg(not(any(soc_has_pmu, esp32c2)))]
    {
        cfg_if::cfg_if! {
            if #[cfg(soc_has_lpwr)] {
                ...
                let rtc_cntl = crate::peripherals::RTC_CNTL::regs();
            }
        }
        ...
        unsafe {
            cfg_if::cfg_if! {
                if #[cfg(soc_has_apb_ctrl)] {
                    ...
                    let syscon = crate::peripherals::SYSCON::regs();
                }
            }
...
pub unsafe extern "C" fn queue_create(queue_len: u32, item_size: u32) -> *mut c_void {
    super::compat::queue::queue_create(queue_len as i32, item_size as i32).cast()
}
...
pub unsafe extern "C" fn queue_delete(queue: *mut c_void) {
    super::compat::queue::queue_delete(queue.cast());
}
...
pub unsafe extern "C" fn queue_send(
    ...
    super::compat::queue::queue_send_to_back(
        queue.cast(),
        item.cast_const(),
...
pub unsafe extern "C" fn queue_send_from_isr(
    ...
    super::compat::queue::queue_try_send_to_back_from_isr(
        queue.cast(),
        item.cast_const(),
...
pub unsafe extern "C" fn queue_send_to_front(
    ...
    super::compat::queue::queue_send_to_front(
        queue.cast(),
        item,
        blob_ticks_to_micros(block_time_tick),
    )
...
pub unsafe extern "C" fn queue_recv(
    ...
    super::compat::queue::queue_receive(queue.cast(), item, blob_ticks_to_micros(block_time_ms))
}
...
pub unsafe extern "C" fn queue_recv_from_isr(
    ...
    super::compat::queue::queue_try_receive_from_isr(
        queue.cast(),
        item,
        higher_priority_task_waken.cast(),
    )
...
pub unsafe extern "C" fn queue_msg_waiting(queue: *mut c_void) -> u32 {
    super::compat::queue::queue_messages_waiting(queue.cast())
}
...
pub unsafe extern "C" fn __esp_radio_vTaskDelay(ticks: u32) {
    unsafe {
        super::compat::common::__esp_radio_usleep(super::time::blob_ticks_to_micros(ticks));
    }
}
