...
fn Software0() {
    unsafe {
        let (fnc, arg) = crate::radio::ble::btdm::ble_os_adapter_chip_specific::ISR_INTERRUPT_7;
        trace!("interrupt Software0 {:?} {:?}", fnc, arg);

        if !fnc.is_null() {
            let fnc: fn(*mut crate::radio::sys::c_types::c_void) = core::mem::transmute(fnc);
            fnc(arg);
        }
    }
}
...
extern "C" fn WIFI_MAC() {
    unsafe {
        let (fnc, arg) = crate::radio::wifi::os_adapter::ISR_INTERRUPT_1;
...
extern "C" fn RWBT() {
    unsafe {
        let (fnc, arg) = crate::radio::ble::btdm::ble_os_adapter_chip_specific::ISR_INTERRUPT_5;
...
extern "C" fn BT_BB() {
    unsafe {
        let (fnc, arg) = crate::radio::ble::btdm::ble_os_adapter_chip_specific::ISR_INTERRUPT_8;
