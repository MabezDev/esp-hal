...
#[cfg(not(feature = "ble"))]
use crate::radio::private::InitializationError;
/// System bindings for radio functionality.
pub mod sys {
    cfg_if::cfg_if! {
        if #[cfg(esp32)] {
            pub use esp_wifi_sys_esp32::*;
            pub use esp_wifi_sys_esp32::{c_types, include};
        } else if #[cfg(esp32c2)] {
            pub use esp_wifi_sys_esp32c2::*;
            pub use esp_wifi_sys_esp32c2::{c_types, include};
        } else if #[cfg(esp32c3)] {
            pub use esp_wifi_sys_esp32c3::*;
            pub use esp_wifi_sys_esp32c3::{c_types, include};
        } else if #[cfg(esp32c6)] {
            pub use esp_wifi_sys_esp32c6::*;
            pub use esp_wifi_sys_esp32c6::{c_types, include};
        } else if #[cfg(esp32h2)] {
            pub use esp_wifi_sys_esp32h2::*;
            pub use esp_wifi_sys_esp32h2::{c_types, include};
        } else if #[cfg(esp32s2)] {
            pub use esp_wifi_sys_esp32s2::*;
            pub use esp_wifi_sys_esp32s2::{c_types, include};
        } else if #[cfg(esp32s3)] {
            pub use esp_wifi_sys_esp32s3::*;
            pub use esp_wifi_sys_esp32s3::{c_types, include};
        }
    }
}

use crate::radio::radio::{setup_radio_isr, shutdown_radio_isr};
#[cfg(feature = "wifi")]
use crate::radio::wifi::WifiError;
...
pub(crate) fn init() -> Result<(), InitializationError> {
    ...
    crate::radio::common_adapter::enable_wifi_power_domain();
    ...
    #[cfg(coex)]
    match crate::radio::wifi::coex_initialize() {
        0 => {}
...
pub(crate) fn deinit() {
    ...
    #[cfg(coex)]
    {
        unsafe { crate::radio::wifi::os_adapter::coex_disable() };
        unsafe { crate::radio::wifi::os_adapter::coex_deinit() };
    }
